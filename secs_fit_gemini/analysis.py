...
grid, _ = secs3d.gemini_tools.make_csgrid(xg, height=maph, crop_factor=0.67, #0.2
                        resolution_factor=0.4, extend=extend, extend_ew=3.4, #0.25
                        dlat = 1.5, dlon=-3, dipole_lompe=True, asymres=1.5)

# Get height integrated currents
Je, Jn, mlons, mlats = helpers.height_integtated_current(xg, dat, grid, ENU=ENU, maph=maph)
...
singularity = 0.5
m_cf, m_df = helpers.fit_J(grid, Je, Jn, mlons, mlats, l1=1e-2, singularity=singularity)


#Evaluate CF SECS representation.
Jcf_e, Jcf_n, Jdf_e, Jdf_n, mlon_eval, mlat_eval = helpers.evalJ(grid, m_cf, m_df, maph=maph, extend=extend, singularity=singularity)
...
plt.tight_layout()
fig.savefig('secs_fit_currents.pdf')
...
_times = np.ones(1)*60 #temporal locations to evaluare for FAC [minutes]
...
lonindex = 3
plt.plot(mlats[0,:,lonindex],datadict['J1'].reshape(shape)[0,:,lonindex], label='GEMINI')
